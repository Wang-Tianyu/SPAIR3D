...
def set_EXPERIMENT_FOLDER(path):
...
def init_checkpoint(checkpoint_dir, data_config, model_config, extra_file, resume):
    ...
    :param extra_file: path to extra_file you want to copy.
    ...
    experiment_folders = [f for f in os.listdir(checkpoint_dir) if not f.startswith('_') and not f.startswith('.')]
